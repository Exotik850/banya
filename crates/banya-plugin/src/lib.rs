...
pub use simd_json::{BorrowedValue as JsonValue, json};
use simd_json::{
    OwnedValue, ValueBuilder,
    base::{ValueAsArray, ValueAsObject, ValueAsScalar},
};

pub type PluginResult<'a> = Result<Option<JsonValue<'a>>, String>;
...
pub trait JsonPlugin {
    ...
    fn invoke<'a>(capability: &str, args: JsonValue<'a>) -> PluginResult<'a>;
...
#[must_use]
pub fn plugin_info_from_routes(
...
pub fn dispatch_plugin_capability<'a>(
    capability: &str,
    args: JsonValue<'a>,
    routes: &[CapabilityRoute],
) -> PluginResult<'a> {
...
impl<T: JsonPlugin> Guest for T {
    ...
    fn configure(mut config: Vec<u8>) -> Result<(), String> {
        T::configure(json_from_bytes(&mut config)?)
    }

    fn invoke(capability: String, mut args: Vec<u8>) -> Result<Option<Vec<u8>>, String> {
        let args = json_from_bytes(&mut args)?;
        let value = T::invoke(&capability, args)?;
        value.map(|v| json_ref_to_bytes(&v)).transpose()
    }
...
pub fn json_from_bytes(bytes: &mut [u8]) -> Result<JsonValue<'_>, String> {
    if bytes.is_empty() {
        return Ok(JsonValue::null());
    }
    simd_json::from_slice(bytes).map_err(|e| format!("Failed to parse JSON bytes: {e}"))
}

/// Serialize a JSON value for the WIT boundary.
pub fn json_ref_to_bytes(value: &JsonValue) -> Result<Vec<u8>, String> {
    simd_json::to_vec(value).map_err(|e| format!("Failed to serialize JSON: {e}"))
}

pub fn json_to_bytes(value: &OwnedValue) -> Result<Vec<u8>, String> {
    simd_json::to_vec(value).map_err(|e| format!("Failed to serialize JSON: {e}"))
}
...
pub trait JsonArgsExt {
    fn require_object(&self) -> Result<&simd_json::borrowed::Object<'_>, String>;
    fn require_str(&self, key: &str) -> Result<&str, String>;
    fn optional_str(&self, key: &str) -> Option<&str>;
    fn require_array(&self, key: &str) -> Result<&Vec<JsonValue<'_>>, String>;
}

impl JsonArgsExt for JsonValue<'_> {
    fn require_object(&self) -> Result<&simd_json::borrowed::Object<'_>, String> {
        self.as_object()
...
pub mod prelude {
    ...
    pub use crate::json_ref_to_bytes;
